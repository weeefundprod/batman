...
try:
    ...
    if alreadyExits == False:
        try:
            sql = "INSERT INTO product (SERIAL_NUMBER, MODEL, SKU, VENDOR, SCREEN, PROCESSOR, HHD_SSD, GRAPHIC_CARD, WEBCAM, BLUETOOTH, DVD, INTERNAL_NUMBER) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)"
            val = (serial_number, product, sku, vendor, screen, processor, ', '.join(array_hhd_sdd), graphic_card, webcam, bluetooth, dvd, internal_number)
